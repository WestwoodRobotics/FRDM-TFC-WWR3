...
Modified: Mark Walker
		9/17/16
		For KL25Z microprocessor and TFC library
************************************************************/
...
int errorCalc( void )
{
	...
	{	
		...
		if (avgCamData[i] < minValue && avgCamData[i] > 500)
		{
...
int processCamData( uint16_t* cameraData )
{
	...
	{	
		...
		avgCamData[i] = sum;
		TERMINAL_PRINTF("sum %f \n\r", sum);
	}
	
	// Calculate the error in the current position of the car
	error = errorCalc();
	TERMINAL_PRINTF("line position %f \n\r", error);
	
	// Return the distance from 64 (Center) that the error was
	return (64-error);
}
